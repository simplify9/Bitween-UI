...
const ExchangeDocument: React.FC<Props> = ({downloadUrl, name = ""}) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    //  downloadUrl
    return (
        <div
            className="flex gap-2 py-1 border bg-gray-50 px-2 align-center rounded shadow-sm">
            {
                isModalOpen && <ExchangeDocumentModal downloadUrl={downloadUrl} name={name} onClose={() => setIsModalOpen(false)}/>
            }
            <svg className="w-5 h-90" xmlns="http://www.w3.org/2000/svg"
                 ...
                 fillRule="evenodd" clipRule="evenodd" viewBox="0 0 512 425.29">
                <path
            ...
            <span onClick={() => setIsModalOpen(true)}>{name}</span>
        </div>
